...
### The QzVert Engine (Conversion)
- User pastes a YouTube URL, uploads a PDF, or pastes long text.
- Gemini API analyzes the content and structures it into a **Quest**.
- Output Format:
  - 1 Quest (Topic Level)
  - 3-5 Stages (Chapter Level)
  - 3-5 Quizzes per Stage (Challenge Level)

### The Learning Map (Gamification)
- Visual roadmap showing the stages of the Quest.
- Fog of War: Stages are locked until the previous one is completed (Curiosity).
- Life System: 3 Hearts per stage. Incorrect answers lose a heart (Loss Avoidance).

### Semantic Explore (Discovery)
