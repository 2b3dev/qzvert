  ...
  .handler(async ({ data }): Promise<SaveQuestResult> => {
    ...
    if (data.quest.type === 'quiz') {
      ...
      const questionInserts = data.quest.quizzes.map((quiz, quizIndex) => ({
        ...
        correct_answer:
          quiz.type === 'multiple_choice' ? quiz.correct_answer : 0,
      ...
      for (
        let stageIndex = 0;
        stageIndex < data.quest.stages.length;
        stageIndex++
      ) {
      ...
      for (
        let moduleIndex = 0;
        moduleIndex < data.quest.modules.length;
        moduleIndex++
      ) {
        ...
        if (stageError) {
          throw new Error(
            `Failed to save lesson module: ${stageError.message}`,
          )
        }
      }
    }
...
export const getPublishedActivities = createServerFn({ method: 'GET' }).handler(
  async () => {
    const supabase = getSupabaseFromCookies()
...
export const getActivityById = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    let questions: Array<Database['public']['Tables']['questions']['Row']> = []
    ...
    if (activity.type === 'lesson') {
      ...
      generatedQuest = {
        ...
        modules: stages.map((stage) => ({
          title: stage.title,
          content_blocks: stage.lesson_summary
            ? JSON.parse(stage.lesson_summary)
            : [],
        })),
      }
    } else if (
      activity.type === 'quiz' ||
      (stages.length === 1 && !stages[0].lesson_summary)
    ) {
      // Smart Quiz
      generatedQuest = {
        type: 'quiz' as const,
    ...
    return {
      activity,
      generatedQuest,
      themeConfig: activity.theme_config,
    }
  })
...
export const getUserActivities = createServerFn({ method: 'GET' }).handler(
  async () => {
    const supabase = getSupabaseFromCookies()
...
export const incrementPlayCount = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { error: updateError } = await supabase
      .from('activities')
      .update({ play_count: activity.play_count + 1 })
      .eq('id', data.activityId)
...
export const updateQuest = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<{ success: boolean }> => {
    ...
    const updateData: Database['public']['Tables']['activities']['Update'] = {
      ...
      raw_content: data.rawContent,
      theme_config:
        data.themeConfig as unknown as Database['public']['Tables']['activities']['Update']['theme_config'],
    }
    ...
    if (existingStages && existingStages.length > 0) {
      ...
      // Delete stages
      await supabase.from('stages').delete().eq('activity_id', data.activityId)
    }
...
export const updateAllowedEmails = createServerFn({ method: 'POST' })
  .inputValidator((data: { activityId: string; emails: Array<string> }) => data)
  .handler(async ({ data }) => {
    const supabase = getSupabaseFromCookies()
  ...
  .handler(async ({ data }): Promise<CanUserPlayResult> => {
    ...
    if (!userId) {
      ...
      if (
        ...
        new Date(activity.available_until) < now
      ) {
        return {
          can_play: false,
          reason: 'expired',
  ...
  .handler(async ({ data }): Promise<ActivePlaySession | null> => {
    ...
    if (activity.time_limit_minutes) {
      ...
      remainingSeconds = Math.max(
        0,
        Math.floor((timeLimitMs - elapsedMs) / 1000),
      )
...
export const getUserStats = createServerFn({ method: 'GET' }).handler(
  async (): Promise<UserStats> => {
    ...
    // Calculate stats
    const totalActivitiesPlayed = new Set(records.map((r) => r.activity_id))
      .size
    ...
    const recentPlays = records.slice(0, 5).map((r) => ({
      ...
      activity_title:
        (r.activities as { title: string } | null)?.title || 'Unknown',
      activity_thumbnail:
        (r.activities as { thumbnail: string | null } | null)?.thumbnail ||
        null,
...
export const getActivityResults = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<ActivityResultsResponse> => {
    ...
    const results: Array<ActivityResult> = records.map((r) => ({
      id: r.id,
      activity_id: r.activity_id,
      activity_title:
        (r.activities as { title: string } | null)?.title || 'Unknown',
...
export const getSuggestedActivities = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<Array<SuggestedActivity>> => {
    const supabase = getSupabaseFromCookies()
    ...
    if (keywords.length === 0) {
      ...
      return (popularActivities || []) as Array<SuggestedActivity>
    }
    ...
    const { data: searchResults } = await supabase
      ...
      .eq('status', 'public')
      .or(
        `title.ilike.%${keywords[0]}%,title.ilike.%${keywords[1] || keywords[0]}%`,
      )
      .order('play_count', { ascending: false })
      .limit(limit)

    if (searchResults && searchResults.length >= limit) {
      return searchResults as Array<SuggestedActivity>
    }
    ...
    if (unique.length > 0) {
      return unique as Array<SuggestedActivity>
    }
