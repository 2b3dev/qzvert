...
import type {
  ActivityStatus,
  GeneratedQuiz,
  GeneratedQuestCourse,
} from '../../../types/database'

export const Route = createFileRoute('/activity/upload/quest/$id')({
  component: QuestUploadPage,
})

function QuestUploadPage() {
  ...
  useEffect(() => {
    const loadActivity = async () => {
      ...
      if (isNewActivity) {
        if (currentActivity && currentActivity.type === 'quest') {
          ...
          const loadedQuizzes = currentActivity.stages.flatMap((stage) => stage.quizzes)
      ...
      try {
        ...
        if (data && data.generatedQuest.type === 'quest') {
          const quest = data.generatedQuest as GeneratedQuestCourse
          ...
          setTags(quest.tags || [])
          const loadedQuizzes = quest.stages.flatMap((stage) => stage.quizzes)
          setQuizzes(loadedQuizzes)
          const total = loadedQuizzes.reduce(
            (sum, q) => sum + (q.points ?? 100),
            0,
        ...
        <Card className="max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              This quest doesn't exist or you don't have access
            </p>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // For quest, we need to update stages with the edited quizzes
      const updatedQuest: GeneratedQuestCourse = {
        ...currentActivity,
        title,
        description,
